...
QtFirebase::QtFirebase(QObject* parent) : QObject(parent)
{
    ...
    qWarning() << self << ":QtFirebase(QObject* parent)" ;
...
{
    static int count = 0;
    qWarning() << self << "::waitForFutureCompletion" << "waiting for future" << &future << "completion. Initial status" << future.status();
    while(future.status() == firebase::kFutureStatusPending) {
        QGuiApplication::processEvents();
        count++;

        if(count % 100 == 0)
            qWarning() << count << "Future" << &future << "is still pending. Has current status" << future.status();

        if(count % 200 == 0) {
            qWarning() << count << "Future" << &future << "is still pending. Something is probably wrong. Breaking wait cycle. Current status" << future.status();
            count = 0;
            break;
        }
        QThread::msleep(10);
    ...
    if(future.status() == firebase::kFutureStatusComplete) {
       qWarning() << self << "::waitForFutureCompletion" << "ended with COMPLETE";
    }

    if(future.status() == firebase::kFutureStatusInvalid) {
       qWarning() << self << "::waitForFutureCompletion" << "ended with INVALID";
    }

    if(future.status() == firebase::kFutureStatusPending) {
       qWarning() << self << "::waitForFutureCompletion" << "ended with PENDING";
    }
}
...
{
    qWarning() << self << "::addFuture" << "adding" << eventId;
    ...
    qWarning() << self << "::addFuture" << "starting future watch";
...
void QtFirebase::requestInit()
{
    if(!PlatformUtils::getNativeWindow()) {
        qWarning() << self << "::requestInit" << "no native UI pointer";
        return;
    }
    ...
    if(!_ready) {
        ...
        _ready = true;
        qWarning() << self << "::requestInit" << "initialized";
        emit readyChanged();
    }
}
...
void QtFirebase::processEvents()
{
    ...
    if(_futureMap.isEmpty()) {
        qWarning() << self << "::processEvents" << "stopping future watch";
